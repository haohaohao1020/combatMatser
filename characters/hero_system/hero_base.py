...
from characters.hero_system.character_types import HeroType, HeroArchetype, SkillType
...
class SkillData:
    ...
    def to_attack_data(self) -> AttackData:
        return AttackData(
            attack_type=AttackType.ULTIMATE if self.skill_type == SkillType.ULTIMATE else AttackType.LIGHT,
            damage=self.damage,
            hitbox_x=20,
            hitbox_y=10,
            hitbox_w=80,
...
class HeroStats:
    ...
    double_jump_force: float = DOUBLE_JUMP_FORCE


class HeroCharacter:
    def __init__(self, x: float, y: float, is_player1: bool, hero_type: HeroType):
        ...
        self.special_skill_max_cooldown = 180
        self.is_using_special = False
        self.special_skill_timer = 0
        self.current_special_skill: Optional[SkillData] = None
        ...
        hero_info = self._get_hero_info()
        colors = hero_info.get("colors", {})
        self.body_color = colors.get("primary", (50, 100, 255))
        self.body_color_dark = colors.get("secondary", (30, 60, 180))
        self.accent_color = colors.get("accent", (0, 200, 255))
    ...
    def get_light_attack(self) -> AttackData:
        base_attack = get_light_attack()
        base_attack.damage = int(base_attack.damage * self.stats.attack_multiplier)
        return base_attack

    def get_heavy_attack(self) -> AttackData:
        base_attack = get_heavy_attack()
        base_attack.damage = int(base_attack.damage * self.stats.attack_multiplier)
        return base_attack

    def get_low_attack(self) -> AttackData:
        base_attack = get_low_attack()
        base_attack.damage = int(base_attack.damage * self.stats.attack_multiplier)
        return base_attack

    def handle_input(self, keys, opponent: 'HeroCharacter'):
        if self.is_ai:
            return
        ...
        can_attack = (self.hitstun <= 0 and self.dodge_timer <= 0 and
                       self.grounded_timer <= 0 and not self.is_ultimate and
                       not self.is_using_special)
        ...
        if self.is_player1:
            ...
            if keys[pygame.K_q] and self.special_skill_cooldown <= 0 and can_attack:
                self.start_special_skill()
            ...
            if keys[pygame.K_RCTRL] and self.special_skill_cooldown <= 0 and can_attack:
                self.start_special_skill()

        if not self.is_blocking and not self.is_attacking:
            if self.dodge_timer <= 0:
                if move_input != 0:
                    self.vel_x = move_input * self.stats.walk_speed
    ...
    def jump(self):
        ...
        jump_force = self.stats.jump_force if self.jump_count == 0 else self.stats.double_jump_force
    ...
    def start_special_skill(self):
        ...
        self.is_attacking = True
        self.has_hit = False
        self.current_attack = skill.to_attack_data()
    ...
    def start_ultimate(self):
        ...
        self.current_attack = ultimate_skill.to_attack_data()
    ...
    def apply_damage(self, damage: int, attack: AttackData) -> int:
        reduced_damage = int(damage / self.stats.defense_multiplier)
    ...
    def update_status_effects(self):
        ...
        if self.stunned_timer > 0:
            ...
            self.hitstun = max(self.hitstun, 1)
    ...
    def update(self, opponent: 'HeroCharacter'):
        ...
        if self.special_skill_cooldown > 0:
            self.special_skill_cooldown -= 1

        if self.special_skill_timer > 0:
            self.special_skill_timer -= 1
            if self.special_skill_timer <= 0:
                self.is_using_special = False
    ...
    def _render_by_state(
        ...
        elif self.is_using_special:
            self._render_special_skill(surface, x, render_h, render_y, body_color, body_color_dark)
